...
if(isset($_POST['search']))
{
	...
	$query = "SELECT * FROM `memberasds` WHERE  CONCAT(`trackid`)LIKE '%".$valueToSearch."%'";
  //  $query = "SELECT * FROM `membertracking` WHERE  CONCAT(`mem_id`, `trackid`, `firstname`, `lastname`, `address`)LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);
    
}
    else {
        $query ="SELECT * FROM `memberasds` ORDER BY mem_id DESC";
        $search_result = filterTable($query);
...
<br>
	...
	<form action="indexasds.php" method="post" enctype="multipart/form-data">
...
<br>
  ...
  <a href="transferasds_hrmo.php?mem_id=<?php echo $fetch['mem_id']?>">HRMO</a>
    ...
    <a href="transferasds_sgod.php?mem_id=<?php echo $fetch['mem_id']?>">SGOD</a>
	<a href="transferasds_cid.php?mem_id=<?php echo $fetch['mem_id']?>">CID</a>
	<a href="transferasds_dpsu.php?mem_id=<?php echo $fetch['mem_id']?>">DPSU</a>
	<a href="transferasds_supply.php?mem_id=<?php echo $fetch['mem_id']?>">Supply</a>
...
<center><a href="del_asds.php?mem_id=<?php echo $fetch['mem_id']?>"><i class="fa fa-trash" style='font-size:16px;color:Blue'><font size="4"><B>Del</B></font></i></a></center>
